...
**Strategy**
- Fetch: 언제 프로세스를 메모리에 적재할 것인가?
  - Demand: 요구가 있을 때
  - Anticipatory: 요구가 있을 것을 예상
- Placement: 어디에 적재할 것인가 
- Replacement: 메모리 공간이 부족할 경우 새로 적재돼야 할 프로세스를 위해 공간을 확보하는 기법
- Allocation: 프로세스에게 메모리 공간을 얼마 정도로 줄 것인가 결정
...
## 가상 메모리
프로그램을 작은 조각으로 나누어 그 중에 일부분만을 메모리에 적재(비연속적으로)
이로인하여, 사용자가 메모리의 크기로부터 자유로울 수 있게 된다.
...
**Map Table**
- 가상주소를 실주소와 mapping
- Residence 비트: 이 페이지가 메모리에 적재되어 있는가?

**Translation Lookaside Buffer(TLB)
- Key 값으로 찾고자하는 워드를 동시에 접근하는 연관 메모리

**Page 의 Protection, Sharing**
- Protection Bits 을 통한 접근 제어

### Segmentation
- 논리적인 단위별로 프로그램의 조각을 다른 크기로 나누어 메모리 관리를 수행 
- 세그먼트 테이블의 엔트리는 존재 비트, 세그먼트의 길이, 실주소, 접근 제어 키 등으로 구성 
- 페이징과 다르게 실주소를 계산하는데 쓰이는 프레임 번호를 저장하는 것이 아니라 실주소를 저장

### Paging 을 사용하는 Segmentation
- 페이징과 세그먼테이션의 장점을 함께 가지기 위한 기법으로 세그먼트를 다시 페이지로 분할
- 맵핑을 위한 테이블의 크기가 커지고 실주소로의 접근까지 메모리 접근이 더 요구되는 단점
...
- Optimal(MIN): 페이지 부재를 최소화
  - 가장 오랫동안 참조되지 않을 페이지를 교체
  - 앞으로 어떤 페이지들을 참조할지 미리 알 수 없기 때문 현실적으로 구현이 불가능 
- FIFO: Time Stampin, Queue 를 활용하여 구현
  - FIFO Anomaly: 부재율이 올라가는 모순이 발생할 수 있다
- LRU
- Second-chance: 적재된 후 한번이라도 더 참조된 페이지를 바로 교체시키지 않고 한 번 더 기회를 준다
- NUR(개선된 Second-chance): Clock 기법에 갱신 비트를 추가
- LFU: 참조 횟수를 누적하여 많이 참조된 페이지는 또 참조될 확률이 높을 것이다
- MFU: 많이 참조된 페이지는 더 이상 참조되지 않을 것이다
- Page Buffering

**Working Set**
- 지역성을 표현하는 Working set을 메모리에 유지함으로써 스레싱을 방지
- 가변 할당이 필요

**PFF**
- 부재 사이의 간격으로 Working set을 관리

**Cleaning Strategy**
- 적재 중 내용이 변경된 페이지를 언제 디스크에 기록시킬 것인가?

**Load Control**
- 다중 프로그래밍의 Degree 를 결정